...
const Guest = ({ reservationId }: { reservationId: string }) => {
  ...
  useEffect(() => {
    const fetchGuestInfo = async () => {
      try {
        ...
        const response = await fetch(`/api/Forms/requisitionGuestInfo/${reservationId}`);
        if (response.ok) {
    ...
    fetchGuestInfo();
  }, [reservationId]); // Re-run effect when reservationid changes
  return (
    <>
      ...
      <div className='  flex flex-col mt-5'>
        ...
        <div className="flex gap-10  flex-col ">
          <div className="font-semibold items-center flex gap-4 text-xl text-primary">
            ...
            Review Guest Info From Requistion{" "}
          </div>

        </div>


        <article className="bg-white space-y-4 p-4 shadow-md mt-5">
