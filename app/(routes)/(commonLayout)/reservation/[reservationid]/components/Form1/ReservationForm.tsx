...
const SalesForm: FC<IdProps> = ({ id }) => {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      if (res.ok) {
        setOpen(false);
        router.push(`/reservation/${id}/ItineraryReservation`);
        toast({
  ...
  useEffect(() => {
    const fetchGuestInfo = async () => {
      try {
        ...
        const response = await fetch(`/api/guest/operation`);
        if (response.ok) {
  ...
  return (
    <>
      <section className="flex mt-8  gap-4 h-max">
        ...
        <Form1SideDiv
         assignedTo={apiData?.find(item => item.id === assignedValue)?.name}
